...
def get_lr_segments(reads, start, end, sample_size=50_000):
    ...
    sample_idxs = np.random.choice(np.arange(len(lr_segments)), min(sample_size, len(lr_segments)), replace=False)
...
def run_get_matches(lr_segments, threads):
    ...
    with Pool(threads) as p:
        for match_count, d in tqdm(p.imap(get_matches, lr_segments, chunksize=10), total=len(lr_segments)):
