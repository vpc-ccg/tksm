        ...
        def load_from_file(self, filename, output):
            ...
            with MISC_PY.get_open_func(filename)(filename, "rt") as model_file:
                for line in model_file:
                    ...
                    self.alternatives[kmer] = [
                        ERROR_MODEL_PY.align_kmers(kmer, x[0]) for x in alternatives
                    ]
                    self.probabilities[kmer] = [float(x[1]) for x in alternatives]
                    count += 1
            print(
                f"\r  done: loaded error distributions for {count} {self.kmer_size}-mers",
                file=output,
            )
...
class MISC_PY:
    ...
    @classmethod
    def print_in_two_columns(
        cls, l1p1, l2p1, l3p1, l1p2, l2p2, l3p2, output, space_between=6
    ):
    ...
    def sequence_fragment(
        ...
        while True:
            ...
            for j in range(k_size):
                ...
                # the change.
                if (
                    new_base != fragment_base
                    and fragment_base == new_fragment_bases[i + j]
                ):
                    ...
                    if change_count % SETTINGS_PY.ALIGNMENT_INTERVAL == 0:
                        ...
                        # the result to update the error estimate.
                            ...
                            pos = random.randint(
                                0, frag_len - SETTINGS_PY.ALIGNMENT_SIZE
                            )
                            ...
                            errors = (estimated_errors * weight) + (
                                errors * (1 - weight)
                            )
        ...
        if compute_qscores:
            qual, actual_identity, identity_by_qscores = QSCOREMODEL_PY.get_qscores(
...
class QSCOREMODEL_PY:
    ...
    class QScoreModel(object):
        ...
        def set_up_random_model(self, output):
            ...
            for c in ["=", "X", "I"]:
                ...
                ) = QSCOREMODEL_PY.uniform_dist_scores_and_probs(
                    SETTINGS_PY.RANDOM_QSCORE_MIN, SETTINGS_PY.RANDOM_QSCORE_MAX
                )
        ...
        def set_up_ideal_model(self, output):
            ...
            for c in ["X", "I"]:
                (
                    ...
                    SETTINGS_PY.IDEAL_QSCORE_RANK_1_MIN,
            ...
            (
                ...
                SETTINGS_PY.IDEAL_QSCORE_RANK_2_MIN, SETTINGS_PY.IDEAL_QSCORE_RANK_2_MAX
            )
            (
                self.scores["==="],
                self.probabilities["==="],
            ) = QSCOREMODEL_PY.uniform_dist_scores_and_probs(
                SETTINGS_PY.IDEAL_QSCORE_RANK_3_MIN, SETTINGS_PY.IDEAL_QSCORE_RANK_3_MAX
            )
            (
                self.scores["====="],
                self.probabilities["====="],
            ) = QSCOREMODEL_PY.uniform_dist_scores_and_probs(
                SETTINGS_PY.IDEAL_QSCORE_RANK_4_MIN, SETTINGS_PY.IDEAL_QSCORE_RANK_4_MAX
            )
            (
                self.scores["======="],
                self.probabilities["======="],
            ) = QSCOREMODEL_PY.uniform_dist_scores_and_probs(
                SETTINGS_PY.IDEAL_QSCORE_RANK_5_MIN, SETTINGS_PY.IDEAL_QSCORE_RANK_5_MAX
            )
            (
                self.scores["========="],
                self.probabilities["========="],
            ) = QSCOREMODEL_PY.uniform_dist_scores_and_probs(
                SETTINGS_PY.IDEAL_QSCORE_RANK_6_MIN, SETTINGS_PY.IDEAL_QSCORE_RANK_6_MAX
            )
                ...
                for line in model_file:
                    ...
                    try:
                        ...
                        if k > self.kmer_size:
                            self.kmer_size = k
                        print(
                            "\r  " + cigar + (" " * (last_cigar_len - len(cigar))),
                            file=output,
                            end="",
                        )
                        last_cigar_len = len(cigar)
                        scores_and_probs = [
                            x.split(":") for x in parts[2].split(",") if x
                        ]
                        self.scores[cigar] = [int(x[0]) for x in scores_and_probs]
                        self.probabilities[cigar] = [
                            float(x[1]) for x in scores_and_probs
                        ]
                        count += 1
                    except (IndexError, ValueError):
                        sys.exit(
                            f"Error: {filename} does not seem to be a valid qscore model file"
                        )
                print(
                    f"\r  done: loaded qscore distributions for {count} alignments",
                    file=output,
                )
    ...
    def get_qscores(cls, seq, frag, qscore_model):
        ...
        aligned_seq, aligned_frag, full_cigar = cls.align_sequences_from_edlib_cigar(
            seq, frag, cigar
        )
        ...
        for i in range(unaligned_len):
            ...
            while (
                start < 0 or end >= unaligned_len
            ):  # pull back to a smaller k-mer near the seq ends
                start += 1
                end -= 1
    ...
    class Identities(object):
        def __init__(self, mean, stdev, max_identity, output=sys.stderr):
            ...
            if self.mean == self.max_identity:
                ...
                print(
                    f"Using a constant read identity of {self.mean * 100}%", file=output
                )
            elif self.stdev == 0.0:
                ...
                print(
                    "Generating read identities from a beta distribution:", file=output
                )
                self.beta_a, self.beta_b = IDENTITIES_PY.beta_parameters(
                    mean, stdev, max_identity
                )
                MISC_PY.print_in_two_columns(
                    f"  mean  = {MISC_PY.float_to_str(self.mean * 100):>3}%",
                ...
                QUICKHIST_PY.quickhist_beta(
                    self.beta_a, self.beta_b, self.max_identity, 8, output=output
                )
...
class TAIL_NOISE_MODEL_PY:
    ...
    class KDE_noise_generator:
        ...
        def from_data(cls, mapped, unmapped, lx, ly, transition_matrix, bw, threads=64):
            ...
            full_grid = np.array([[x, y] for x in lx for y in ly]).reshape(
                lx.shape[0], ly.shape[0], 2
            )
            results = Parallel(n_jobs=threads)(
                delayed(kde_vals.score_samples)(row) for row in full_grid
            )
...
def parse_args():
    ...
    if args.badread_mean_identity <= SETTINGS_PY.MIN_MEAN_READ_IDENTITY:
        sys.exit(
            f"Error: mean read identity must be at least {SETTINGS_PY.MIN_MEAN_READ_IDENTITY}"
        )
    if args.badread_max_identity <= SETTINGS_PY.MIN_MEAN_READ_IDENTITY:
        sys.exit(
            f"Error: max read identity must be at least {SETTINGS_PY.MIN_MEAN_READ_IDENTITY}"
        )
...
def badread(molecule_id, raw_seq, compute_qual=False):
    ...
    (seq, quals, actual_identity, _) = SIMULATE_PY.sequence_fragment(
        raw_seq,
        target_identity,
        error_model,
        qscore_model,
...
if __name__ == "__main__":
    ...
    if args.badread:
        identities = IDENTITIES_PY.Identities(
