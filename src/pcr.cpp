...
public:
    PCR(int cycles, double efficiency, double error_rate, std::mt19937 &rand_gen)
        : cycles{cycles},
          ...
          error_rate{(4 * error_rate) / 3},  // Our error definition is a base to be assigned to random base not
                                             // changing to other bases so we are adjusting the probability
          rand_gen{rand_gen} {}

    void do_pcr(ostream &ost, const molecule_descriptor &md, int step, const vector<int> &positions,
                vector<std::pair<int, char>> mutations, double drop_ratio) {
        ...
        std::sample(positions.begin(), positions.end(), std::back_inserter(mutation_pos), (int)mutation_count,
                    rand_gen);
    ...
    int perform(const vector<molecule_descriptor> &molecules, int number_of_target_reads, ostream &ost) {
        ...
        int64_t molecule_count = std::accumulate(
            molecules.begin(), molecules.end(), 0,
            [](int64_t sum, const molecule_descriptor &cpy) -> int64_t { return sum + cpy.get_depth(); });
...
class PCR_module::impl : public tksm_module {
    cxxopts::ParseResult parse(int argc, char **argv) {
        ...
        for (const auto &p : pcr_presets) {
            preset_string += ("- " + p.first + ": " + "error-rate: " + std::to_string(p.second.first) +
                              ", efficiency-rate: " + std::to_string(p.second.second) + "\n");
        }
