...
public:
    ...
    custom_distribution(const json &json_obj) : uniform_dist{0.0, 1},
        pdfv{json_obj["data"].begin(), json_obj["data"].end()},
        cdfv{0},
        bins{json_obj["labels"].begin(), json_obj["labels"].end()}
    {

        double sum_pdf = std::accumulate(pdfv.begin(), pdfv.end(), RealType{0});
        for (double d : pdfv) {
    ...
    template <class Generator>
    static void forward(Generator &g){
        g();g();
    }
...
public:
    ...
    custom_distribution2D(const json &json_obj) : uniform_dist{0.0, 1}{
        vector<unsigned long> shape = json_obj["shape"];
        vector<double> pdata = json_obj["data"];
        

        x_axis_index = vector<IndexType> { json_obj["labels"].begin(), json_obj["labels"].begin() + shape[0]};
        y_axis_index = vector<IndexType> {json_obj["labels"].begin() + shape[0], json_obj["labels"].begin() + shape[0] + shape[1]};
    ...
    cxxopts::ParseResult parse(int argc, char **argv) {
        ...
        options.add_options("main")
            (
                ...
                "kde-model",
                "[model].json generated by the truncation modelling",
                cxxopts::value<string>()
            )(
                "always-end",
...
public:
    ...
    auto get_dist()
        ...
        if (args["kde-model"].count() > 0) {
            std::string kdefile_json = args["kde-model"].as<string>();
            return custom_distribution2D<>{kdefile_json};
        }
    ...
    static auto truncate_transformer_kde(auto &disko, auto &sider_decider, auto &rand_gen, bool always_end,
        ...
        return std::ranges::views::transform([&, always_end, models_length](auto &md) {
            ...
            auto side_ratio = [always_end, &sider_decider, &rand_gen]() {
                if (always_end && !sider_decider) {
                    custom_distribution<>::forward(rand_gen);
                    return 1.0;
                }
                else {
                    return sider_decider.value()(rand_gen);
                }
            }();
    ...
    int run() {
        ...
        if (args["kde-model"].count() > 0) {
            ...
            ifstream json_stream {args["kde-model"].as<string>()};
            auto matrices_json = json::parse(json_stream);

            auto disko = [] (const auto &matrices_json) -> std::optional<custom_distribution2D<>>{
                for(const auto &part : matrices_json){
                    if (part["name"] == "KDE_mtx"){
                        return part;
                    }
                }
                return std::nullopt;
            ...
            auto sider_decider = [] (const auto &matrices_json) -> std::optional<custom_distribution<double,double>>{
                for(const auto &part : matrices_json){
                    if (part["name"] == "end_mtx"){
                        return part;
                    }
                }
                return std::nullopt;
            ...
            for (const auto &md : stream_mdf(args["input"].as<string>(), true) |
                                      truncate_transformer_kde(*disko, sider_decider,
    ...
    void describe_program() {
        ...
        if (args.count("kde-model")) {
            logi("KDE model file: {}", args["kde-model"].as<string>());
        }
