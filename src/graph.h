...
public:
    ...
    public:
        ...
        public:
            ...
            nei_iter(graph &owner, size_t target, decltype(owner.arcs[0].begin()) index)
                : owner(owner), target(target), index(index) {}
            auto &operator*() { return owner.nodes[index->first].first; }
            nei_iter &operator++() {
                ++index;
                return *this;
            }
            ...
            friend bool operator==(const nei_iter &a, const nei_iter &b) {
                return a.target == b.target && a.index == b.index;
            };
            friend bool operator!=(const nei_iter &a, const nei_iter &b) { return !(a == b); };
        };
        nei_iter begin() { return nei_iter(owner, target); }

        nei_iter end() { return nei_iter(owner, target, owner.arcs[target].end()); }
    };
    ...
    bool in(const N &key) { return reverse_index.find(key) != reverse_index.end(); }
    A &value(const N &key) {
        size_t index = reverse_index[key];
        assert(in(key));
