...
def format_gnu_time_string(
    ...
    fields = list()
    fields.append(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    fields.append(f"{exprmnt}")
    fields.append(f"{prefix}")
    fields.append(f"{process}")
    fields.append("%e")
    fields.append("%U")
    fields.append("%M")
    ...
    return f'$(which time) -f "{fields}" -o >({awk_cmd} >> {{input.time}}) '
